...
export function JournalEntryEditDialog({ open, onOpenChange, entry, rate }: Props) {
  ...
  useEffect(() => {
    ...
    setExchangeRate(Number(entry.exchange_rate) || rate);
    const srcLines = entry.journal_entry_lines || [];
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const { error: jeErr } = await supabase.from('journal_entries').update({
        date,
      ...
      toast.success('Asiento actualizado — reportes financieros actualizados');
      onOpenChange(false);
    } catch (e: any) {
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-2xl max-h-[85vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>Modifica el asiento y sus líneas. Los reportes financieros se actualizarán automáticamente.</DialogDescription>
        </DialogHeader>
